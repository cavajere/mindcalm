      ...
      <table class="w-full">
        <thead class="bg-gray-50">
          <tr>
            ...
            <th class="text-center px-4 py-3 text-xs font-medium text-text-secondary uppercase">Categoria</th>
            <th class="text-center px-4 py-3 text-xs font-medium text-text-secondary uppercase">Livello</th>
            <th class="text-center px-4 py-3 text-xs font-medium text-text-secondary uppercase">Durata</th>
            <th class="text-center px-4 py-3 text-xs font-medium text-text-secondary uppercase">Delivery</th>
        ...
        <tbody class="divide-y divide-gray-50">
          <tr v-for="audio in audioItems" :key="audio.id" class="hover:bg-gray-50/50">
            ...
            <td class="px-4 py-3 text-center">
              <span v-if="audio.category" class="inline-flex text-xs px-2 py-1 rounded-full" :style="{ backgroundColor: (audio.category.color || '#4A90D9') + '20', color: audio.category.color || '#4A90D9' }">
                {{ audio.category.name }}
              </span>
            </td>
            <td class="px-4 py-3 text-center text-sm text-text-secondary">{{ getAudioLevelLabel(audio.level) }}</td>
            <td class="px-4 py-3 text-center text-sm text-text-secondary">{{ formatDuration(audio.durationSec) }}</td>
            <td class="px-4 py-3">
            ...
            <td class="table-actions-cell">
              <div class="table-actions-group">
                <button
                  ...
                  v-tooltip="audio.status === 'PUBLISHED' ? 'Riporta a bozza' : 'Pubblica audio'"
                  :aria-label="audio.status === 'PUBLISHED' ? 'Riporta a bozza' : 'Pubblica audio'"
                >
                ...
                <button
                  ...
                  class="icon-action-button icon-action-button-neutral"
                  v-tooltip="'Modifica audio'"
                  aria-label="Modifica audio"
                >
